...
// Fetches on mount (and whenever deps change) and exposes a stable
// refetch. `fetcher` is intentionally not in the dep array — deps
// describe what makes the request distinct (e.g. the insights range).
...
// Paginated history. `fetchMore` appends the next cursor page; the
// initial load happens on mount (and when `limit` changes).
